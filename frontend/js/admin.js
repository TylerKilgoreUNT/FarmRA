...
function bindSideActionButtons() {
	...
	sideButtons.forEach((btn) => {
		btn.addEventListener("click", () => {
			...
			const topOffset = 92;
			const targetTop = targetSection.getBoundingClientRect().top + globalThis.scrollY - topOffset;
...
function bindAdminForm() {
	...
	if (refreshUsersBtn) {
		refreshUsersBtn.addEventListener("click", async () => {
			...
			showStatus("API refresh partially unavailable. Showing local data where needed.", "warning");
		});
	}
}
...
function normalizeUser(entry) {
	...
	const email = String(
		entry.email || entry.googleEmail || entry.google_email || entry.u_email || "",
	).trim();

	const rawGateways =
		entry.gateways || entry.gatewayIds || entry.gateway_ids || entry.nodes || [];
...
function normalizeGatewayCollection(payload) {
	const candidates = Array.isArray(payload)
		? payload
		: payload?.gateways || payload?.nodes || payload?.items || payload?.data || [];
...
function normalizeGateway(entry) {
	...
	const label =
		String(entry.label || entry.name || entry.gatewayName || entry.gateway_name || "").trim() ||
		`Gateway ${id}`;
...
function renderGatewayList() {
	...
	const gateways = state.gateways.length > 0 ? state.gateways : readLocalGateways();
	if (gateways.length === 0) {
...
function writeLocalGateways(gateways) {
	...
	try {
		globalThis.localStorage.setItem(LOCAL_GATEWAYS_KEY, JSON.stringify(gateways));
	} catch (error) {
...
function setButtonBusy(button, isBusy) {
	...
	button.textContent = button.dataset.originalText || button.textContent || "Save";
